...
namespace airDucts
{
	class Troinik
	{
		...
		public void createTroinikKrPr(double dlin,double shir, double vys1,double diam, double vys2,double zazor, IModelDoc2 Part)
		{
			boolstatus = Part.Extension.SelectByID2("Front Plane", "PLANE", 0, 0, 0, false, 0, null, 0);
			Part.ClearSelection2(true);
			Part.SketchManager.InsertSketch(true);
			...
			boolstatus = Part.Extension.SelectByID2("Point1", "SKETCHPOINT", -diam / 2, 0, vys1, true, 0, null, 0);
			boolstatus = Part.Extension.SelectByID2("Point3", "SKETCHPOINT", 0, 0, vys1, true, 0, null, 0);
			Part.SketchAddConstraints("sgHORIZONTALPOINTS2D");
			Part.ClearSelection2(true);

			boolstatus = Part.Extension.SelectByID2("Point1", "SKETCHPOINT", -diam / 2, 0, vys1, false, 0, null, 0);
			boolstatus = Part.Extension.SelectByID2("Point2", "SKETCHPOINT", 0, diam / 2, vys1, true, 0, null, 0);
			...
			boolstatus = Part.Extension.SelectByID2("Эскиз2", "SKETCH", -diam / 2, 0, vys1, true, 1, null, 0);
			...
			refPlane = (RefPlane)Part.FeatureManager.InsertRefPlane(8, diam / 2 + zazor, 0, 0, 0, 0);
			Part.ClearSelection2(true);

			//boolstatus = Part.Extension.SelectByID2("Плоскость5", "PLANE", 0, 0, 0, false, 0, null, 0);
			//Part.SketchManager.InsertSketch(true);

			//skSegment = (SketchSegment)Part.SketchManager.CreateLine( -zazor * 5,-vys1/2 - dlin/2, 0, -shir / 2, -vys1 / 2 - dlin / 2, 0);
			//skSegment = (SketchSegment)Part.SketchManager.CreateLine(-shir / 2, -vys1 / 2 - dlin / 2, 0, -shir / 2, -vys1 / 2 + dlin / 2, 0);
			//skSegment = (SketchSegment)Part.SketchManager.CreateLine(-shir / 2, -vys1 / 2 + dlin / 2, 0, shir / 2, -vys1 / 2 + dlin / 2, 0);
			//skSegment = (SketchSegment)Part.SketchManager.CreateLine(shir / 2, -vys1 / 2 + dlin / 2, 0, shir / 2, -vys1 / 2 - dlin / 2, 0);
			//skSegment = (SketchSegment)Part.SketchManager.CreateLine(shir / 2, -vys1 / 2 - dlin / 2, 0, 0, -vys1 / 2 - dlin / 2, 0);
			//Part.ClearSelection2(true);

			//boolstatus = Part.Extension.SelectByID2("Point6", "SKETCHPOINT", 0, -vys1 / 2 - dlin / 2, 0, true, 0, null, 0);
			//boolstatus = Part.Extension.SelectByID2("Point1@Исходная точка", "EXTSKETCHPOINT", 0, 0, 0, true, 0, null, 0);
			//Part.SketchAddConstraints("sgVERTICALPOINTS2D");
			//Part.ClearSelection2(true);

			//boolstatus = Part.Extension.SelectByID2("Line3", "SKETCHSEGMENT", 0, -vys1 / 2 + dlin / 2, 0, true, 0, null, 0);
			//Part.SelectMidpoint();
			//boolstatus = Part.Extension.SelectByID2("Point1@Исходная точка", "EXTSKETCHPOINT", 0, 0, 0, true, 0, null, 0);
			//Part.SketchAddConstraints("sgVERTICALPOINTS2D");
			//Part.ClearSelection2(true);

			//boolstatus = Part.Extension.SelectByID2("Point1", "SKETCHPOINT", -zazor * 5, -vys1 / 2 - dlin / 2, 0, false, 0, null, 0);
			//boolstatus = Part.Extension.SelectByID2("Point6", "SKETCHPOINT", 0, -vys1 / 2 - dlin / 2, 0, true, 0, null, 0);

			//Part.AddDimension2(-diam * 2 / 3, 0, 2.5);
			//Part.ClearSelection2(true);

			//var myDimension5 = Part.Parameter("D1@Эскиз5");
			//myDimension5.SystemValue = zazor;
			//Part.SketchManager.InsertSketch(true);
			...
			Part.SketchManager.InsertSketch(true);
			boolstatus = Part.Extension.SelectByID2("Плоскость5", "PLANE", 0, 0, 0, false, 0, null, 0);
			vskLines = Part.SketchManager.CreateCenterRectangle(0, -vys1 / 2, 0, shir/2-0.001, -vys1 / 2 + dlin / 2, 0);
			Part.ClearSelection2(true);
			Part.SketchManager.InsertSketch(true);
		...
		public void createVrezPr(double dlin, double shir, double vys1,  double zazor, IModelDoc2 Part)
		{
			boolstatus = Part.Extension.SelectByID2("Справа", "PLANE", 0, 0, 0, false, 0, null, 0);
			Part.SketchManager.InsertSketch(true);
			...
			skSegment = (SketchSegment)Part.SketchManager.CreateLine(dlin / 2, -shir / 2, 0.0, -dlin / 2, -shir / 2, 0.0);
			skSegment = (SketchSegment)Part.SketchManager.CreateLine(-dlin / 2, -shir / 2, 0.0, -dlin / 2, -zazor * 10, 0.0);
			Part.ClearSelection2(true);

			boolstatus = Part.Extension.SelectByID2("Point1", "SKETCHPOINT", -dlin / 2, 0, 0, true, 0, null, 0);
			boolstatus = Part.Extension.SelectByID2("Point1@Исходная точка", "EXTSKETCHPOINT", 0, 0, 0, true, 0, null, 0);
			Part.SketchAddConstraints("sgHORIZONTALPOINTS2D");
			Part.ClearSelection2(true);
						
			boolstatus = Part.Extension.SelectByID2("Point1", "SKETCHPOINT", -dlin / 2, 0, 0, false, 0, null, 0);
			boolstatus = Part.Extension.SelectByID2("Point6", "SKETCHPOINT", -dlin / 2, -zazor * 10, 0, true, 0, null, 0);

			Part.AddDimension2(-dlin * 2, 0, 0);
			Part.ClearSelection2(true);

			var myDimension5 = Part.Parameter("D1@Эскиз1");
			myDimension5.SystemValue = zazor;
			Part.ClearSelection2(true);
			Part.SetPickMode();
			Part.ClearSelection2(true);
			...
			customBendAllowanceData = Part.FeatureManager.CreateCustomBendAllowance();
			customBendAllowanceData.KFactor = 0.5;
			feat = Part.FeatureManager.InsertSheetMetalBaseFlange2(zazor, false, zazor, vys1, 0,
				false, 0, 0, 1, customBendAllowanceData, false, 0, 0.0001, 0.0001, 0.5, true, false, true, true);

		}

		public void createVrezKr( double diam, double vys1, double zazor, IModelDoc2 Part)
		{
			boolstatus = Part.Extension.SelectByID2("Справа", "PLANE", 0, 0, 0, false, 0, null, 0);
			Part.SketchManager.InsertSketch(true);
			skSegment = (SketchSegment)Part.SketchManager.CreateArc(0, 0, 0, -diam / 2, 0, 0, 0, diam / 2, 0, 1);

			Part.AddDimension2(-diam * 2 / 3, diam, 0);
			//Part.InsertGtol();
			Part.ClearSelection2(true);

			var myDimension1 = Part.Parameter("D1@Эскиз1");
			myDimension1.SystemValue = diam / 2;
			...
			customBendAllowanceData = Part.FeatureManager.CreateCustomBendAllowance();
			customBendAllowanceData.KFactor = 0.5;
			feat = Part.FeatureManager.InsertSheetMetalBaseFlange2(zazor, false, 0.00015, vys1, 0, false, 0, 0, 0, customBendAllowanceData, false, 0, 0.0001, 0.0001, 0.5, true, false, true, true);

		}

		public void createVrezKr2(double diam, double vys1, double zazor, IModelDoc2 Part)
		{
			boolstatus = Part.Extension.SelectByID2("Справа", "PLANE", 0, 0, 0, false, 0, null, 0);
			Part.SketchManager.InsertSketch(true);
			skSegment = (SketchSegment)Part.SketchManager.CreateArc(0, 0, 0, -diam / 2, 0, 0, 0, diam / 2, 0, 1);
			...
			customBendAllowanceData = Part.FeatureManager.CreateCustomBendAllowance();
			customBendAllowanceData.KFactor = 0.5;
			feat = Part.FeatureManager.InsertSheetMetalBaseFlange2(zazor, false, 0.00015, vys1, vys1, false, 0, 0, 0, customBendAllowanceData, false, 0, 0.0001, 0.0001, 0.5, true, false, true, true);

		}

		public void createVrezPr2(double dlin, double shir, double vys1, double zazor, IModelDoc2 Part)
		{
			boolstatus = Part.Extension.SelectByID2("Справа", "PLANE", 0, 0, 0, false, 0, null, 0);
			Part.SketchManager.InsertSketch(true);
			...
			customBendAllowanceData = Part.FeatureManager.CreateCustomBendAllowance();
			customBendAllowanceData.KFactor = 0.5;
			feat = Part.FeatureManager.InsertSheetMetalBaseFlange2(zazor, false, zazor, vys1, vys1,
				false, 0, 0, 0, customBendAllowanceData, false, 0, 0.0001, 0.0001, 0.5, true, false, true, true);
